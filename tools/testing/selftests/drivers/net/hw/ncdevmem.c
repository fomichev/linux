...
static int bind_tx_queue(unsigned int ifindex, int socket_fd, int dmabuf_fd,
			 struct ynl_sock **ys)
{
	...
	*ys = ynl_sock_create(&ynl_netdev_family, &yerr);
	...
	netdev_bind_tx_req_set_socket_fd(req, socket_fd);
...
static int do_client(struct memory_buffer *mem)
{
	...
	if (bind_tx_queue(ifindex, socket_fd, mem->fd, &ys))
		error(1, 0, "Failed to bind\n");
