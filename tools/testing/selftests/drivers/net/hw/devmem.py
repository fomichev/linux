...
def check_tx(cfg) -> None:
    ...
    with bkg(listen_cmd, exit_wait=True) as socat:
        wait_port_listen(port)
        cmd(f"echo -e \"hello\\nworld\"| {cfg.bin_remote} -f {cfg.ifname} -s {cfg.v6} -p {port}", host=cfg.remote, shell=True)
...
def check_txrx(cfg) -> None:
    ...
    with bkg(listen_cmd, exit_wait=True) as socat:
        wait_port_listen(port)
        cmd(f"cat random_file.txt | {cfg.bin_remote} -f {cfg.ifname} -s {cfg.v6} -p {port}", host=cfg.remote, shell=True)
